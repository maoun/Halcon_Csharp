...
namespace MyNet.Atmcs.Uscmcp.Data
{
    public class UserManagerNew : IUserManagerNew
    {
        ...
        public UserManagerNew(string dataAccessName)
        {
        ...
        public DataTable GetSerUserInfo(string systemId, string where, string ip)
        {
            ...
            try
            {
                mySql = string.Format(@"select a.*,b.USERNAME,b.REGDATE,c.DEPARTNAME,d.ROLECODE,b.PASSWORD, e.usercode,
...
(SELECT * FROM  t_cfg_role h  WHERE systemId='00') i ON  i.rolecode=d.ROLECODE where a.systemid='{1}' and {2}", ip, systemId, where);
                ...
                //  MyNet.Common.Data.DataAccess data = new MyNet.Common.Data.DataAccess(MyNet.Common.Data.DataAccess.DatabaseType.MySql, "Server=10.2.111.228;Port=3306;Database=zltest;Uid=root;Pwd=srt1234;");
                //  DataTable ds = data.Get_DataTable("SELECT   location_id FROM  t_cfg_location ");
                //mySql = "SELECT CODE,codedesc FROM t_sys_code f WHERE f.codetype='011005'";
                DataTable dt = dataAccess.Get_DataTable(mySql);
                // MySqlHelper.Get_DataTable(mySql);

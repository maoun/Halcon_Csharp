...
// ***********************************************************************
// <copyright file="VideoTemplate.aspx.cs" company="ZKLT">
...
namespace MyNet.Atmcs.Uscmcp.Web
{
    ...
    /// Class VideoTemplate.
    /// </summary>
    public partial class VideoTemplate : System.Web.UI.Page
    {
        #region 成员变量
        ...
        protected void ButOKClick(object sender, DirectEventArgs e)
        {
            try
            {
                if (Session["templatepage"] != null)
                {
                    ...
                    if (res > 0)
                    {
                        ...
                        {
                            case "VideoTemplate1":
                                userinfo.VideoTemplate1 = templateInfo.TemplatePage;
                                break;

                            case "VideoTemplate2":
                                userinfo.VideoTemplate2 = templateInfo.TemplatePage;
                                break;
                        }
            ...
            catch (Exception ex)
            {
                ...
                logManager.InsertLogError("VideoTemplate.aspx-ButOKClick", ex.Message+"；"+ex.StackTrace, "ButOKClick has an exception");
            }
        }
        ...
        protected void TemplateGroup_Event(object sender, DirectEventArgs e)
        {
            ...
            {
                ILog.WriteErrorLog(ex);
                logManager.InsertLogError("VideoTemplate.aspx-TemplateGroup_Event", ex.Message+"；"+ex.StackTrace, "TemplateGroup_Event has an exception");
            }
        }
        ...
        protected void InitForm()
        {
            try
            {
                ...
                panelVideo.AutoLoad.Url = "../Common/CSBrowse.aspx?funcid=010102";
                panelVideo.Reload();
            }
            catch (Exception ex)
            {
                ILog.WriteErrorLog(ex);
                logManager.InsertLogError("VideoTemplate.aspx-InitForm", ex.Message+"；"+ex.StackTrace, "InitForm has an exception");
            }
        }
        ...
        public string ConvertUrl(string url)
        {
            if (url.IndexOf("pages/collect") >= 0)
            {
                ...
                {
                    ILog.WriteErrorLog(ex);
                    logManager.InsertLogError("VideoTemplate.aspx-ConvertUrl", ex.Message+"；"+ex.StackTrace, "ConvertUrl has an exception");
                    return url;
                }
            }
            return url;

...
{
    public partial class PeccancySingleRepeatCheck : System.Web.UI.Page
    {
        #region 成员变量
        ...
        protected void ButCheckOkClick(object sender, DirectEventArgs e)
        {
            try
            {
                ...
                Hashtable pecInfo = GetPeccancyInfo("1", "3");
                ...
                if (tgsDataInfo.UpdatePeccancyInfo(pecInfo) > 0)
                {
                    ...
                    //FrmClear();
                    logManager.InsertLogRunning(UserLogin.GetUserName(), "复审有效：号牌种类[" + cmbPlateType.SelectedItem.Text + "] ,号牌号码[" + VehicleHead.VehicleText + txtPlateId.Text + "],违法记录Id[" + pecInfo["xh"].ToString() + "] ,违法时间[" + pecInfo["wfsj"].ToString() + "] ,违法地点[" + cmbLocation.SelectedItem.Text + "] ,违法行为[" + cmbPeccancyType.SelectedItem.Text + "]", ipaddress, "7", VehicleHead.VehicleText + txtPlateId.Text, CurrentId.Value.ToString());
                }
                else
                {
                    logManager.InsertLogRunning(UserLogin.GetUserName(), "复审有效失败：号牌种类[" + cmbPlateType.SelectedItem.Text + "] ,号牌号码[" + VehicleHead.VehicleText + txtPlateId.Text + "] ,违法记录Id[" + pecInfo["xh"].ToString() + "] ,违法时间[" + pecInfo["wfsj"].ToString() + "] ,违法地点[" + cmbLocation.SelectedItem.Text + "] ,违法行为[" + cmbPeccancyType.SelectedItem.Text + "]", ipaddress, "7", VehicleHead.VehicleText + txtPlateId.Text, CurrentId.Value.ToString());
                }
            }
            catch (Exception ex)
            {
                logManager.InsertLogError("PeccancySingleRepeatCheck.aspx-ButCheckOkClick", ex.Message + "；" + ex.StackTrace, "ButCheckOkClick has an exception");
            }
        }
        ...
        protected void ButCheckNoClick(object sender, DirectEventArgs e)
        {
            try
            {
                Hashtable pecInfo = GetPeccancyInfo("2", "4");
                string ipaddress = Request.ServerVariables.Get("Remote_Addr").ToString();
                if (tgsDataInfo.UpdatePeccancyInfo(pecInfo) > 0)
                {
                    NextInfo();
                    ...
                    //FrmClear();
                    logManager.InsertLogRunning(UserLogin.GetUserName(), "复审无效：号牌种类[" + cmbPlateType.SelectedItem.Text + "] ,号牌号码[" + VehicleHead.VehicleText + txtPlateId.Text + "] ,违法记录Id[" + pecInfo["xh"].ToString() + "] ,违法时间[" + pecInfo["wfsj"].ToString() + "] ,违法地点[" + cmbLocation.SelectedItem.Text + "] ,违法行为[" + cmbPeccancyType.SelectedItem.Text + "]", ipaddress, "6", VehicleHead.VehicleText + txtPlateId.Text, CurrentId.Value.ToString());
                }
                else
                {
                    logManager.InsertLogRunning(UserLogin.GetUserName(), "复审无效失败：号牌种类[" + cmbPlateType.SelectedItem.Text + "] ,号牌号码[" + VehicleHead.VehicleText + txtPlateId.Text + "] ,违法记录Id[" + pecInfo["xh"].ToString() + "] ,违法时间[" + pecInfo["wfsj"].ToString() + "] ,违法地点[" + cmbLocation.SelectedItem.Text + "] ,违法行为[" + cmbPeccancyType.SelectedItem.Text + "]", ipaddress, "6", VehicleHead.VehicleText + txtPlateId.Text, CurrentId.Value.ToString());
                }
            }
            catch (Exception ex)
            {
                logManager.InsertLogError("PeccancySingleRepeatCheck.aspx-ButCheckNoClick", ex.Message + "；" + ex.StackTrace, "ButCheckNoClick has an exception");
            }
        }
        ...
        {
            Hashtable pecInfo = GetPeccancyInfo("3", "1");
            if (tgsDataInfo.UpdatePeccancyInfo(pecInfo) > 0)
            {
                NextInfo();
                string ipaddress = Request.ServerVariables.Get("Remote_Addr").ToString();
        ...
        private void SetControlValue(int index)
        {
            ...
            if (dt2 != null && dt2.Rows.Count > 0)
            {
                ...
                if (dt3 != null)
                {
                    ...
                    //lbTxzzt.Attributes["Style"] = "color:green;";
                ...
                else
                {
                    ...
                    // lbTxzzt.Attributes["Style"] = "color:red;";
                ...
                TxtShyh.Text = dt2.Rows[0]["col53"].ToString();
                TxtZqmj.Text = dt2.Rows[0]["col22"].ToString();
                ...
                if (string.IsNullOrEmpty(TxtZqmj.Text))
                {
                    this.TxtZqmj.Text = userInfo.UserName;
                }
                string shbj = dt2.Rows[0]["col31"].ToString();
                switch (shbj)
                {
                    case "0":
                        PanAmply.Title = "当前状态:<font color='#008000'>&nbsp;&nbsp;" + dt2.Rows[0]["col20"].ToString() + "</font>";
                        break;
                    ...
                    case "2":
                        PanAmply.Title = "当前状态:<font color='#ff0000'>&nbsp;&nbsp;" + dt2.Rows[0]["col20"].ToString() + "</font>";
                        break;
                }
                //string js = "getTxzzt();";
                //ResourceManager1.RegisterAfterClientInitScript(js);
            }
        }
        ...
        /// <param name="jcbj"></param>
        /// <returns></returns>
        private Hashtable GetPeccancyInfo(string jzzt, string jcbj)
        {
            UserInfo userInfo = System.Web.HttpContext.Current.Session["userinfo"] as UserInfo;
            ...
            pecInfo.Add("jzzt", jzzt);
            pecInfo.Add("jzsj", string.Format("{0:yyyy-MM-dd HH:mm:ss}", DateTime.Now));
            pecInfo.Add("jzyh", userInfo.UserCode);//
            pecInfo.Add("jcbj", jcbj);
            pecInfo.Add("xh", CurrentId.Value.ToString());
            pecInfo.Add("zqmj", userInfo.UserPolice);//
            return pecInfo;
        }

<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PassCarSpeedCount.aspx.cs" Inherits="MyNet.Atmcs.Uscmcp.Web.PassCarSpeedCount" %>
...
<body>
    <form id="form1" runat="server">
        <ext:ResourceManager ID="ResourceManager1" runat="server" DirectMethodNamespace="PassCarSpeedCount" />
        ...
        <ext:Viewport ID="Viewport2" runat="server" Layout="border">
            <Items>
                <ext:FormPanel ID="Panel1" Region="North" runat="server"
                    ...
                    <TopBar>
                        <ext:Toolbar ID="Toolbar1" runat="server">
                            <Items>
                                <ext:Label ID="lblStartTime" runat="server" Text='<%# GetLangStr("PassCarOcrCount1","统计类型：") %>' StyleSpec="margin-left:10px;">
                                </ext:Label>
                                <ext:ComboBox ID="CmbCountType" runat="server" Width="140">

                                    <Items>
                                        <ext:ListItem Text='<%# GetLangStr("PassCarOcrCount2","按平均速度统计") %>' Value="0" AutoDataBind="true" />
                                        <ext:ListItem Text='<%# GetLangStr("PassCarOcrCount3","按最高速度统计") %>' Value="1" AutoDataBind="true" />
                                    </Items>
                                </ext:ComboBox>
                                <ext:Label ID="Label2" runat="server" Text='<%# GetLangStr("PassCarOcrCount4","统计时间：") %>' StyleSpec="margin-left:10px;">
                                </ext:Label>
                                <ext:ComboBox ID="CmbYear" runat="server" Editable="false" DisplayField="col1" ValueField="col0"
                                    TypeAhead="true" Mode="Local" ForceSelection="true" EmptyText='<%# GetLangStr("PassCarOcrCount5","选择年...") %>' SelectOnFocus="true"
                                    Width="80">
                                ...
                                <ext:ComboBox ID="CmbMonth" runat="server" Editable="false" DisplayField="col1" ValueField="col0"
                                    TypeAhead="true" Mode="Local" ForceSelection="true" EmptyText='<%# GetLangStr("PassCarOcrCount6","选择月...")%>' SelectOnFocus="true"
                                    Width="60">
                                ...
                                <ext:ComboBox ID="CmbDay" runat="server" Editable="false" DisplayField="col1" ValueField="col0"
                                    TypeAhead="true" Mode="Local" ForceSelection="true" EmptyText='<%# GetLangStr("PassCarOcrCount7","选择日...") %>' SelectOnFocus="true"
                                    Width="60">
                                ...
                                <ext:Label ID="Label4" runat="server" Text='<%# GetLangStr("PassCarOcrCount8","卡口名称：") %>' StyleSpec="margin-left:10px;">
                                </ext:Label>
                                <ext:DropDownField ID="FieldStation" runat="server"
                                    ...
                                    <Component>
                                        <ext:TreePanel runat="server" Height="400" Shadow="None" ID="TreeStation"
                                            ...
                                            <Buttons>
                                                <ext:Button runat="server" Text='<%# GetLangStr("PassCarOcrCount9","清除") %>'>
                                                    <Listeners>
                                                        <Click Handler="clearSelect(TreeStation,FieldStation);" />
                                                    </Listeners>
                                                </ext:Button>
                                                <ext:Button runat="server" Text='<%# GetLangStr("PassCarOcrCount10","关闭") %>'>
                                                    <Listeners>
                                ...
                                <ext:Button ID="TbutQuery" runat="server" Icon="ControlPlayBlue" Text='<%# GetLangStr("PassCarOcrCount11","查询") %>'>
                                    <DirectEvents>
                                ...
                                <ext:Button ID="ButReset" runat="server" Icon="ControlBlank" Text='<%# GetLangStr("PassCarOcrCount12","重置") %>'>
                                    <DirectEvents>
                ...
                <ext:Panel ID="Panel2" runat="server" Region="Center" DefaultBorder="false" AutoScroll="true">
                        ...
                        <ext:Toolbar ID="Toolbar2" runat="server">
                            <Items>
                                <ext:TabStrip ID="TabStrip1" runat="server">
                                    <Items>
                                        <ext:TabStripItem ActionItemID="pnlData" runat="server" Title='<%# GetLangStr("PassCarOcrCount13","图示统计") %>' AutoDataBind="true" />
                                        <ext:TabStripItem ActionItemID="GridFlow" runat="server" Title='<%# GetLangStr("PassCarOcrCount14","列表统计") %>' AutoDataBind="true" />
                                    </Items>
                                </ext:TabStrip>
                                <ext:ToolbarSeparator ID="ToolbarSeparator1" runat="server" />
                                <%-- <ext:Button ID="ButPrint" runat="server" Icon="Printer" ToolTip='<%# GetLangStr("PassCarOcrCount15","统计打印") %>'>
                                    <DirectEvents>
                                        <Click OnEvent="ButPrintClick" />
                                    </DirectEvents>
                                </ext:Button>--%>
                                <ext:Button ID="ButExcel" runat="server" AutoPostBack="true" OnClick="ButExcelClick" ToolTip='<%# GetLangStr("PassCarOcrCount19","导出Excel") %>'
                                    Icon="PageExcel">
                                </ext:Button>
                                <%--    <ext:Button ID="ButChart" runat="server" Icon="Report" ToolTip='<%# GetLangStr("PassCarOcrCount16","图表打印") %>'>
                                    <DirectEvents>
                    ...
                    <Items>
                        <ext:GridPanel ID="GridFlow" runat="server" StripeRows="true" Title='<%# GetLangStr("PassCarOcrCount17","列表统计") %>' Collapsible="true"
                        ...
                        <ext:Panel ID="pnlData" runat="server" Title='<%# GetLangStr("PassCarOcrCount18","图示统计") %>' AutoRender="true" AutoScroll="true">
                            <Content>
                                <center>
